...
#import "AnimationTikTok.h"

#import "AnimationMask.h"

@interface AnimationInteractiveTransition () <UIGestureRecognizerDelegate>
...
@implementation AnimationInteractiveTransition{
...
+ (instancetype)shareInteractiveTransition{
    ...
    static AnimationInteractiveTransition * _shareInteractiveTransition = nil;
    
    static dispatch_once_t onceToken;
    
    dispatch_once(&onceToken, ^{
        _shareInteractiveTransition = [AnimationInteractiveTransition new];
    });
...
- (void)handleAnimatedTransitioning{
    ...
    if (self.type == KAnimationTransitionTypeNone){
        ...
        _animtedTransitioning = [[AnimationSmooth alloc] initWithIsBack:self.isBack];
        
    }else if(self.type == KAnimationTransitionTypeMagicMove){
        
        _animtedTransitioning = [[AnimationMagicMove alloc] initWithIsBack:self.isBack];
        
    }else if(self.type == KAnimationTransitionTypeTikTokComment){
        
        _animtedTransitioning = [[AnimationTikTok alloc] initWithIsBack:self.isBack];
        
    }else if(self.type == KAnimationTransitionTypeMask){
        
        _animtedTransitioning = [[AnimationMask alloc] initWithIsBack:self.isBack];
    }
}
...
//    NSLog(@"state = %ld \n",(long)gesture.state);
    ...
    switch (gesture.state){
        ...
        default:{
            self.isInteractive = NO;
            if (rate >= ScreenEdgePanMoveScale)
                [self finishInteractiveTransition];
            else
                if (velocity > ScreenEdgePanMoveVelocity)
                    [self finishInteractiveTransition];
                else
